...
[이전 글](https://minboykim.github.io/)에서 설명했던 LCA 알고리즘의 방법 중 두번째 알고리즘을 설명하겠다. 첫번째 방법은 이해도 쉽고 구현도 간단하나 최종 O(NM)의 시간복잡도로 시간이 너무 오래 걸린다는 단점이 있다. 이를 최적화 하는 방법을 알아보자.
...
`parent[J][I]의 값이 J번째 노드의 (2^J)번째 부모노드`라고 하자. 위 규칙의 점화식을 써보면
...
첫번째 for문은 두 노드의 깊이를 맞추는 과정이다. 첫번째 알고리즘에서는 하나씩 올라갔지만 2의 제곱수만큼씩 올라가본다. i를 N의 log부터 0까지 (0포함) 1씩 감소시키면서 a와 b의 깊이 차이가 1 << i 보다 크거나 같으면 b를 parent[b][i]로 업데이트한다. (1 << i 는 2^i 를 나타내는 것이다.) 
...
LCA 두번째 알고리즘이 발상과 구현은 조금더 어려우나 시간관점에서 많은 절약을 할 수 있음을 알 수있다. 두 알고리즘 모두 연습이 많이 필요할 것이다.
